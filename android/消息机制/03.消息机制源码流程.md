...
- `sendMessageAtTime()` 方法中在处理 `Message` 时，最终调用的是 `enqueueMessage()` 方法
    ...
    ```
    - 因为存在多个线程往同一个 Loop 线程的 MessageQueue 中插入消息的可能，所以 `enqueueMessage()` 内部需要进行同步。可以看出 MessageQueue 内部是以链表的结构来存储 Message 的（**Message.next**），根据 Message 的延时时间的长短来将决定其在消息队列中的位置。[博客](https://github.com/yangchong211/YCBlogs)
